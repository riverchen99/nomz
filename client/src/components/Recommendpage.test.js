...
describe('Select components and Recommendations page state', () => {
  it('day select has correct initial value', async () => {
    const component = await mount(<MemoryRouter><Recommendpage /></MemoryRouter>);
    expect(component.find('Select#daysel').props().value.value).toEqual('today');
  });
  it('day select updates parent state', async () => {
    const component = await mount(<MemoryRouter><Recommendpage /></MemoryRouter>);
  ...
  it('recommendee select has correct initial value', async () => {
    const component = await mount(<MemoryRouter><Recommendpage /></MemoryRouter>);
    expect(component.find('Select#recsel').props().value.value).toEqual('everyone');
  });
  it('recommendee select updates parent state', async () => {
    const component = await mount(<MemoryRouter><Recommendpage /></MemoryRouter>);
